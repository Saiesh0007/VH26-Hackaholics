...
SECRET_KEY = os.getenv("SECRET_KEY", "ff123")
...
def get_current_user(
    ...
    try:
        auth_user = db.supabase.auth.get_user(credentials.credentials).user
    except Exception:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or expired token", headers={"WWW-Authenticate": "Bearer"})
    profile = db.get_user_by_id(str(auth_user.id))
    if not profile:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="User profile not found")
    return {"sub": profile["id"], "email": profile["email"], "name": profile["name"], "role": profile["role"]}
