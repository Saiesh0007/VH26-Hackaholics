...
class EvaluatorAgent {
  ...
  String _latestVerdict =
      'Baseline verified. System within normal operational bounds.';
  ...
  List<EvaluationAuditReport> get auditHistory =>
  ...
  void captureCheckpoint({
    ...
    _latestVerdict =
        'Evaluating Optimizer changes against checkpoint #${_activeCheckpoint!.id}...';
  }
  ...
  bool evaluateAndAudit(
      PipelineRuntime runtime, PipelineMetrics currentMetrics) {
    ...
    final isCriticalSlaBreached =
        post.p0LatencyMs > 75.0 || post.criticalEventsLost > 0;
    ...
    if (isCriticalSlaBreached || isDegrading) {
      ...
      final improvementDetails =
          'OPTIMAL APPROVED: System stabilized. P0 Latency ${post.p0LatencyMs.toStringAsFixed(1)}ms (SLA preserved), Queue Pressure ${post.queuePressurePercentage.toStringAsFixed(1)}%.';
  ...
  void forceRollback(PipelineRuntime runtime) {
    if (_activeCheckpoint != null) {
      ...
      _status = EvaluationStatus.rolledBack;
      _latestVerdict =
          'MANUAL REVERT: User executed immediate rollback to checkpoint #${_activeCheckpoint!.id}.';
      _activeCheckpoint = null;
    }
  }
}
