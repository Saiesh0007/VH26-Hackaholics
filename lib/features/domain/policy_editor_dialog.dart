...
class _PolicyEditorDialogState extends State<PolicyEditorDialog> {
  ...
  void initState() {
    super.initState();
    _domainNameController = TextEditingController(text: widget.initialPolicy.domainName);
    _descriptionController = TextEditingController(text: widget.initialPolicy.description);
                        ...
                        Row(
                          children: [
                            ...
                            DropdownButton<String>(
                              ...
                              items: ['P0', 'P1', 'P2', 'P3'].map((p) {
                                return DropdownMenuItem(
                                  value: p,
                                  child: Text(p, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                                );
                              }).toList(),
                              onChanged: (newP) {
                                if (newP != null) {
                                  _updateEvent(
                                    ...
                                    evt.copyWith(
                                      priority: newP,
                                      isCritical: newP == 'P0',
                                      canShed: newP == 'P0' ? false : evt.canShed,
                                      canDefer: newP == 'P0' ? false : evt.canDefer,
                                    ),
                                  );
                                }
                              },
                            ...
                            DropdownButton<String>(
                              ...
                              dropdownColor: AppColors.surfaceElevated,
                              underline: const SizedBox.shrink(),
                              items: ['stream', 'micro_batch', 'batch', 'defer'].map((s) {
                                return DropdownMenuItem(
                                  value: s,
                                  child: Text(s.toUpperCase(), style: const TextStyle(fontSize: 11)),
                                );
                              }).toList(),
                              onChanged: (newS) {
                                if (newS != null) {
                                  _updateEvent(index, evt.copyWith(preferredStrategy: newS));
                                }
                              },
                            ),
                          ],
                        ...
                        Row(
                          children: [
                            ...
                            if (!evt.isCritical) ...[
                              Row(
                                children: [
                                  const Text('Sheddable:', style: TextStyle(fontSize: 11, color: AppColors.textMuted)),
                                  Switch(
                                    value: evt.canShed,
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        _validationError!,
                        style: const TextStyle(fontSize: 11, color: AppColors.critical, fontWeight: FontWeight.w600),
                      ),
                    ),
                  ],
                ),
