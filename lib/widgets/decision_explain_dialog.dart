...
class DecisionExplainDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxWidth = (viewport.width - 32).clamp(0.0, 480.0).toDouble();
            ...
            children: [
                      ...
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.check_rounded,
                              color: AppColors.healthy, size: 14),
                        ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(
                      color: AppColors.cardBorder.withOpacity(0.6), width: 0.8),
                ),
                child: Wrap(
                  spacing: 18,
                  runSpacing: 10,
                  ...
                  children: [
                    _telemetryItem(
                        'Latency', '${event.latencyMs.toStringAsFixed(1)} ms'),
                    _telemetryItem('Queue Time',
                        '${event.queueTimeMs.toStringAsFixed(1)} ms'),
  ...
  List<String> _generateExplanations(PipelineEvent evt, bool isCritical) {
    ...
    if (isCritical) {
      ...
      list.add(
          'SLA remaining: ~${(100 - evt.latencyMs).clamp(20, 100).toInt()}ms (strict delivery bound)');
      list.add(
          'Batching rejected because SLA risk is high for critical stream');
      list.add(
          'Shedding prohibited by immutable safety policy (Zero-loss guarantee)');
      list.add('Dedicated priority worker concurrency allocated');
    } else {
      list.add(
          'Non-critical event category (${evt.priority.name.toUpperCase()})');
      if (evt.strategy == ProcessingStrategy.batch) {
        list.add(
            'Micro-batch strategy applied to maximize downstream I/O throughput');
        list.add(
            'SLA permits queuing buffer window without user-facing disruption');
        list.add(
            'Throughput improvement estimated at +400% vs individual streaming');
      } else if (evt.strategy == ProcessingStrategy.defer) {
        list.add(
            'Event deferred to absorb queue backlog during transient surge');
        list.add(
            'Policy canDefer=true; buffered in memory queue for off-peak ingestion');
      } else if (evt.strategy == ProcessingStrategy.shed) {
        list.add('Shedding active according to domain policy threshold');
        list.add(
            'Dropped safely to preserve compute resources for P0 critical transactions');
      } else {
